...
function chomp_remove_menu_pages () {
     remove_menu_page( 'link-manager.php' );
     remove_menu_page( 'edit-comments.php' );
}

add_action( 'admin_menu', 'chomp_remove_menu_pages' );
...
function chomp_unregister_widgets () {
...
add_action( 'widgets_init', 'chomp_unregister_widgets' );
 ...
 */
function chomp_post_type_columns ( $col ) {
...
add_filter( 'manage_posts_columns', 'chomp_post_type_columns' );
add_filter( 'manage_pages_columns', 'chomp_post_type_columns' );

/**
 * Add ID column to Posts and Pages listing
 */
function chomp_fill_id_column ( $col, $id ) {
    global $post;
 ...
 */
function chomp_remove_tinymce_emoji ( $plugins ) {
    if ( !is_array( $plugins ) ) {
        return array();
    }
...
function chomp_remove_emoji () {
    ...
    add_filter( 'tiny_mce_plugins', 'chomp_remove_tinymce_emoji' );
}

add_action( 'init', 'chomp_remove_emoji' );
...
add_action( 'init', 'chomp_update_post_type_features' );
...
add_action( 'after_switch_theme' , 'chomp_set_permalinks', 10, 2 );
...
function chomp_update_mime_types ( $mimes ) {
    $mimes['svg'] = 'image/svg+xml';

    return $mimes;
}

add_filter( 'upload_mimes', 'chomp_update_mime_types' );
 ...
 ******************************************************************************/
function chomp_increase_excerpt_priority( $post ) {
...
add_action( 'edit_form_after_title', 'chomp_increase_excerpt_priority' );
 ...
 ******************************************************************************/
function chomp_force_excerpt_to_show($user_login, $user) {
...
add_action( 'wp_login', 'chomp_force_excerpt_to_show', 10, 2 );
...
function chomp_stop_core_updates ( $a ) {
    global $wp_version;
